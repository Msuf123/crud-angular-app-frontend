...
export class LoginComponent { 
  ...
  constructor(private foms:FormBuilder,private request:FetchDataService,private router:Router){
    ...
    const searchPrmas=new URLSearchParams({client_id:'',scope:'user:email'})//github
    const googleSeachPrams=new URLSearchParams({client_id:'',redirect_uri:'http://localhost:4200/google/callback',scope:'https://www.googleapis.com/auth/userinfo.email',response_type:'token'})
